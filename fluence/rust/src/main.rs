...
fn shamir () -> String {
    ...
    // Attempt to restore the secret from the remaining shares
    // let mut restored = combine_shares(&shares).unwrap().expect("Failed to combine shares");
...
fn shamirGen(priv_key: String) -> (String, String, String, String) {
    ...
    (hex_shares[0], hex_shares[1], hex_shares[2], hex_shares[3]) 
}

fn main() {
    let (sum, count, average) = shamirGen("972146b21b16f89ac82e1331adcd9f09927f99ce54f78122bb6a41cad1c1fe24");
    // Print the restored hex string
    println!("Restored Hex String: {}", sum);
}
